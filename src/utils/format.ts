...
const getBigIntCompat = (value: BigNumberish | BigNumber) => {
  if (value instanceof BigNumber) {
    return getBigInt(value.toBigInt());
  }
...
const _formatBlock = object({
  ...
  difficulty: getBigIntCompat,

  gasLimit: getBigIntCompat,
  gasUsed: getBigIntCompat,

  stateRoot: allowNull(formatHash, null),
  receiptsRoot: allowNull(formatHash, null),

  blobGasUsed: allowNull(getBigIntCompat, null),
  excessBlobGas: allowNull(getBigIntCompat, null),

  miner: allowNull(getAddress),
  extraData: formatData,

  baseFeePerGas: allowNull(getBigIntCompat),
});
...
const _formatTransactionReceipt = object(
  {
    ...
    cumulativeGasUsed: getBigIntCompat,
    effectiveGasPrice: allowNull(getBigIntCompat),
    blobGasPrice: allowNull(getBigIntCompat, null),
...
export function formatTransactionResponse(
  ...
  if (value.to && getBigIntCompat(value.to) === BN_0) {
  ...
  const result = object(
    {
      ...
      gasPrice: allowNull(getBigIntCompat),
      maxPriorityFeePerGas: allowNull(getBigIntCompat),
      maxFeePerGas: allowNull(getBigIntCompat),
      maxFeePerBlobGas: allowNull(getBigIntCompat, null),

      gasLimit: getBigIntCompat,
      to: allowNull(getAddress, null),
      value: getBigIntCompat,
      nonce: getNumber,
      data: formatData,

      creates: allowNull(getAddress, null),

      chainId: allowNull(getBigIntCompat, null),
    },
    {
      data: ['input'],
      gasLimit: ['gas'],
  ...
  */
  ...
  if (result.blockHash && getBigIntCompat(result.blockHash) === BN_0) {
